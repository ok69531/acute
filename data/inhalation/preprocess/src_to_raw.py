...
with open('../scrap/tg403_page_src.json', 'r') as file:
...
'''
    ...
    4. aerosol, vapour, gas, dust, mist가 포함되어있지 않으면 (ex 'other:') 많은 쪽으로 포함
'''

'''
    확인해볼 사항
    1. nan의 수: 82개, unique한 CarRN은 82개 중 51개
    2. inhalation만 있는 데이터의 수: 451
    3. inhalation이 포함된 데이터의 수: 6677
    4. aerosol, vapour, gas, dust, mist가 포함되어있지 않은 데이터의 수
    
    *** inhalation만 있거나 other: 형식의 데이터를 nan으로 처리하면 전체 nan 데이터의 수: 
'''

df = result.copy()

# df[df['Route of administration'] == 'inhalation']
# df[df['Route of administration'].map(lambda x: 'inhalation' in str(x))]['Route of administration'].unique()
# df[df['Route of administration'].map(lambda x: ':' in str(x))]['Route of administration'].unique()
